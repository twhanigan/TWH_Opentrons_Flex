...
def run(protocol: protocol_api.ProtocolContext):
    ...
    protocol.comment("This protcol runs pcr assay for mycoplasma contamination. Have a nice day!")
    ...
    p50_multi.distribute(
        22.5,
        temp_adapter['A6'],
        [pcr_plate[well].bottom(z=0.1) for well in mastermix_wells],
        new_tip='once',
        disposal_vol=5,
        rate=0.5
    )

    #Add the positive/negative control (moved to after mastermix addition)
    ...
    # Transfer negative control to B1–B3 (moved to after mastermix addition)
    p50_multi.distribute(protocol.params.volume_sample,
                         temp_adapter['A5'],
    ...
    # Transfer samples (moved to after mastermix addition)
    for sample_idx in range(protocol.params.num_samples):
    ...
    protocol.comment('Running thermocycler for 3 hrs')
